...
const output = (value: string, unit: VolumeUnit) => value.trim() === '' ? null : (Number.isFinite(Number(value)) && Number(value) >= 0 ? displayVolumeToOz(Number(value), unit) : null)
export function parsePastEventDraft(draft: PastEventDraft, now: number, volumeUnit: VolumeUnit = 'oz') {
  ...
  if (draft.kind === 'feed') { const feed = parseManualFeedDraft(draft, volumeUnit); return feed.ok && feed.entry.endedAt <= now ? { ok: true as const, event: { kind: 'feed' as const, event: feed.entry } } : feed.ok ? { ok: false as const, reason: 'future-date' as const } : feed }
  ...
  const leftOunces = output(draft.pumpLeftOunces, volumeUnit), rightOunces = output(draft.pumpRightOunces, volumeUnit)
