...
function TrackerApp({ authUser, onLogout, babies, selectedBabyId, onSelectedBabyIdChange, onCreateBaby, onRenameBaby, onArchiveBaby }: TrackerAppProps) {
  ...
  // While the caregiver brief is on screen its needs list already surfaces every
  // reminder, so the floating banner only earns its slot once a timer takes over.
  const timingActive = Boolean(trackViewProps.hero.session || trackViewProps.hero.tummySession || trackViewProps.hero.pumpSession)
