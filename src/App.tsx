  ...
  return (
    <main className={`app app-shell ${activeWorkspace === 'care' ? 'workspace-care' : ''}`}>
      ...
      <div className="app-shell-content">
        <header className="workspace-topbar"><div><span className="workspace-eyebrow">{activeWorkspace === 'care' ? 'Care workspace' : activeWorkspace === 'stats' ? 'Insights' : 'Live care'}</span><h1>{activeWorkspace === 'care' ? 'Care' : activeWorkspace === 'stats' ? 'Patterns & progress' : <>Next <span className="workspace-next-time">{trackViewProps.hero.nextFeedWindowText} {trackViewProps.hero.nextFeedSideText}</span></>}</h1></div><div className="workspace-topbar-actions"><span className="sync-pill sync-synced">Online</span>{activeWorkspace !== 'care' ? <CareNotificationCenter notifications={careNotifications} /> : null}{babies.length > 1 ? <select aria-label="Active baby" value={selectedBabyId} onChange={(event) => onSelectedBabyIdChange(event.target.value)}>{babies.map((baby) => <option key={baby.id} value={baby.id}>{baby.name}</option>)}</select> : null}</div></header>
