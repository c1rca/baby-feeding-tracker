...
import { confirmPasswordReset, fetchGoogleAuthStatus, requestEmailLogin, requestPasswordReset, requestTextLogin } from './authApi'
...
type AuthMode = 'phone' | 'code' | 'password' | 'signup' | 'reset'
...
export function LoginScreen({ pending, error, onLogin, onTextLogin }: LoginScreenProps) {
  ...
  const sendPhone = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMode('code')
    setStatus(`Text sent to ${result.maskedPhone}. Tap the link or enter the code below.`)
  }
  ...
  const sendEmail = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!result.ok) return setStatus(result.error)
    setMode('code')
    setStatus(`Email sent to ${result.maskedEmail}. Tap the link or enter the code below.`)
  }
  ...
  return (
    <main className="app auth-screen">
      ...
      <section className="card login-card premium-login-card">
        <div className="login-hero-mark"><span className="brand-mark"><Baby size={24} /></span><span>Feedr</span></div>
        <h1>{mode === 'signup' ? 'Create your account in one step' : 'Sign in fast'}</h1>
        <p className="login-meta">No password required. We send a secure link and a 6-digit code so tired parents can get in fast.</p>
        <div className="login-benefits" aria-hidden="true"><span><MessageCircle size={15} /> Text link</span><span><Mail size={15} /> Email backup</span><span><ShieldCheck size={15} /> Remembered</span></div>
        ...
        {mode === 'phone' ? (
          <form onSubmit={sendPhone} className="frictionless-form">
            ...
            <button className="primary-magic-button" type="submit" disabled={localPending}>{localPending ? 'Sending…' : 'Text me my link'}</button>
          </form>
        ) : null}

        {mode === 'signup' ? (
          <form onSubmit={sendEmail} className="frictionless-form">
            ...
            <button className="primary-magic-button" type="submit" disabled={localPending}>{localPending ? 'Sending…' : 'Email my sign-in link'}</button>
            <p className="login-meta mini">We’ll ask for an optional password and baby details after you’re safely inside.</p>
          </form>
        ) : null}
        ...
        {mode === 'password' ? (
          <form onSubmit={passwordLogin} className="frictionless-form">
        ...
        {mode === 'reset' ? (
          <div className="frictionless-form">
        ...
        <div className="auth-secondary-actions">
          {mode !== 'phone' ? <button className="auth-mode-toggle" type="button" onClick={() => setMode('phone')}>Use mobile number</button> : null}
          {mode !== 'signup' ? <button className="auth-mode-toggle" type="button" onClick={() => setMode('signup')}>Create account</button> : null}
