...
function TimelineList({ items, actions, resumeIds }: { items: TimelineItem[]; actions: TimelineActions; resumeIds: Set<string> }) {
  return <ul className="timeline">{items.map((item) => {
    ...
    if (item.kind === 'feed') return <EntryTimelineItem key={item.entry.id} entry={item.entry} showInlineResume={resumeIds.has(item.entry.id)} actions={actions} />
    return null
  })}</ul>
}

export function Timeline({ now, entries, diapers, medicines, tummyTimes, pumpEvents, editing, editingDiaper, editingMedicine, editingTummyTime, editingPump, openEntryMenuId, confirmingDeleteEntryId, setEntries, setEditing, setEditingDiaper, setEditingMedicine, setEditingTummyTime, setEditingPump, setOpenEntryMenuId, setConfirmingDeleteEntryId, resumeEntry, resumeTummyTime, resumePumpEvent, deleteEntry, deleteDiaper, deleteMedicine, deleteTummyTime, deletePump, startMedicineEdit, startTummyTimeEdit, startPumpEdit, toggleEditingDiaperKind, toggleEditingEntryDiaperKind, saveDiaperEdit, saveMedicineEdit, saveTummyTimeEdit, savePumpEdit, onLogPastEvent, showToast }: TimelineProps) {
  ...
  const resumeIds = new Set(visible.flatMap((item) => item.kind === 'feed' && new Date(item.time).toDateString() === todayKey && (item.entry.leftSeconds > 0 || item.entry.rightSeconds > 0) ? [item.entry.id] : []).slice(0, 2))
  const actions: TimelineActions = { editing, editingDiaper, editingMedicine, editingTummyTime, editingPump, openEntryMenuId, confirmingDeleteEntryId, setEntries, setEditing, setEditingDiaper, setEditingMedicine, setEditingTummyTime, setEditingPump, setOpenEntryMenuId, setConfirmingDeleteEntryId, resumeEntry, resumeTummyTime, resumePumpEvent, deleteEntry, deleteDiaper, deleteMedicine, deleteTummyTime, deletePump, startMedicineEdit, startTummyTimeEdit, startPumpEdit, toggleEditingDiaperKind, toggleEditingEntryDiaperKind, saveDiaperEdit, saveMedicineEdit, saveTummyTimeEdit, savePumpEdit, showToast }
