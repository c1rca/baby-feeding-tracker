...
import { diaperLabel, formatDateInput, formatTimeInput, parseDateAndTime } from '../../domain/trackerDomain'
import { bottleContentLabel } from '../../domain/labels'
import { useUnits } from '../../state/unitPreferencesContext'
import { displayVolumeToOz, formatVolume, volumeUnitName, type VolumeUnit } from '../../domain/units'
  ...
  return (
    <div className="edit-panel">
      <div className="manual-grid">
        <label>Date<input type="date" aria-label="Entry date" value={editing.date} onChange={(event) => actions.setEditing({ ...editing, date: event.target.value })} /></label>
        ...
        <label>Bottle {volumeUnitName(units.volume)}<input inputMode="decimal" value={editing.bottleOunces} onChange={(event) => actions.setEditing({ ...editing, bottleOunces: event.target.value })} placeholder={units.volume === 'ml' ? 'e.g. 75' : 'e.g. 2.5'} /></label>
      ...
      <div className="row">
        <button className="primary" onClick={() => saveEntryEdit(entry, editing, actions, units.volume)}><Save size={15} /> Save</button>
...
function saveEntryEdit(entry: Entry, editing: NonNullable<EditingState>, actions: TimelineActions, volumeUnit: VolumeUnit) {
  const startedAt = editing.date === formatDateInput(entry.startedAt) ? entry.startedAt : parseDateAndTime(editing.date, formatTimeInput(entry.startedAt))
  if (startedAt === null) return actions.showToast('Enter a valid entry date')
  ...
  const bottle = Number(editing.bottleOunces) > 0 ? displayVolumeToOz(Number(editing.bottleOunces), volumeUnit) : null
  const type: FeedType = bottle && leftSeconds + rightSeconds > 0 ? 'mixed' : bottle ? 'bottle' : 'breast'
  const duration = Math.max(0, entry.endedAt - entry.startedAt)
  actions.setEntries((prev) => prev.map((current) => current.id === entry.id ? { ...current, startedAt, endedAt: startedAt + duration, type, leftSeconds, rightSeconds, bottleOunces: bottle, note: editing.note.trim(), diaperKinds: editing.diaperKinds } : current).sort((a, b) => b.endedAt - a.endedAt))
...
export function FeedMetricChips({ entry }: { entry: Entry }) {
  ...
  return (
    <div className="timeline-metrics" aria-label="Feed details">
      ...
      {hasBottle ? <span className={`metric bottle-metric ${total === 0 ? 'primary-metric' : ''}`}>{formatVolume(entry.bottleOunces ?? 0, units.volume)}{entry.bottleContent ? ` ${bottleContentLabel(entry.bottleContent).toLowerCase()}` : ''}</span> : null}
    </div>
  )
}
