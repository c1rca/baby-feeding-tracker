...
type AdditionalOptionsProps = Pick<HeroPanelProps, 'session' | 'additionalOptionsOpen' | 'setAdditionalOptionsOpen' | 'tummySession' | 'setTummySession' | 'setBottleOpen' | 'setManualOpen' | 'logDiaperKinds' | 'logMedicine' | 'logTummyTimeMinutes' | 'startTummyTime' | 'stopTummyTime' | 'startSleep' | 'stopSleep'> & {
  pumpSession: PumpSession | null
  startPumping: (side: 'left' | 'both' | 'right') => void
  startManualPumping: () => void
  stopPumping: () => void
  setPumpingSide: (side: 'left' | 'both' | 'right') => void
...
export function AdditionalOptions({ session, tummySession, setBottleOpen, setManualOpen, logDiaperKinds, logMedicine, logTummyTimeMinutes, startTummyTime, stopTummyTime, startSleep, pumpSession, startPumping, startManualPumping, stopPumping, setPumpingSide, savePumping, pumpCompletionOpen, setPumpCompletionOpen }: AdditionalOptionsProps) {
  ...
  const startSleepFromLauncher = () => { if (!session && !tummySession) startSleep(); else setSheet('tummy') }
  const selectLauncher = (kind: typeof launcherItems[number][0]) => {
    if (kind === 'sleep') return startSleepFromLauncher()
  ...
  return <section className="care-actions" aria-label="Care actions">
    ...
    {sheet === 'diapers' ? <div className="care-popover" role="dialog" aria-label="Log diaper"><div><strong>Diaper change</strong><button type="button" aria-label="Close diaper menu" onClick={close}><X size={16} /></button></div><p>What changed?</p><div className="care-quick-grid"><button onClick={() => logDiaper(['wet'])}><Droplets size={17} />Wet</button><button onClick={() => logDiaper(['stool'])}>Stool</button><button onClick={() => logDiaper(['wet', 'stool'])}>Both</button></div></div> : null}
    {sheet === 'medicine' ? <div className="care-popover" role="dialog" aria-label="Log medicine"><div><strong>Medicine</strong><button type="button" aria-label="Close medicine menu" onClick={close}><X size={16} /></button></div><p>Log a dose now</p><div className="care-quick-grid"><button onClick={() => { logMedicine('tylenol'); close() }}>Tylenol</button><button onClick={() => { logMedicine('motrin'); close() }}>Motrin</button><button onClick={() => { logMedicine('vitamin_d'); close() }}>Vitamin D</button></div></div> : null}
