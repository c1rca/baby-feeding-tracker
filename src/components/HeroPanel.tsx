...
export const HeroPanel = forwardRef<HTMLElement, HeroPanelProps>(function HeroPanel({
  ...
  return (
    <section className="card hero" ref={ref}>
      ...
      {tummySession || pumpSession ? <div className="row hero-actions care-active-actions"><button type="button" className="success end-feed" aria-label={tummySession ? `Stop & save ${tummySession.kind === 'sleep' ? 'Sleep' : 'Tummy Time'}` : 'Finish & add output'} onClick={() => { if (tummySession) stopTummyTime(); else stopPumping() }}>{tummySession ? `Stop & Save ${tummySession.kind === 'sleep' ? 'Sleep' : 'Tummy Time'}` : 'Finish & Add Output'}</button><button type="button" className={`active-clear-link ${careClearConfirming ? 'confirming' : ''}`} aria-label={careClearConfirming ? 'Confirm clear active timer' : 'Clear active timer'} onClick={() => careClearConfirming ? clearCare() : setCareClearConfirming(true)}><XCircle size={14} /> {careClearConfirming ? 'Confirm clear' : 'Clear active'}</button></div> : null}
