...
export const HeroPanel = forwardRef<HTMLElement, HeroPanelProps>(function HeroPanel({
  ...
  return (
    <section className="card hero" ref={ref}>
      ...
      <TimerCluster session={session} activeSeconds={activeSeconds} activeSide={activeSide} suggestedSide={suggestedSide} tummySession={tummySession} tummyActiveSeconds={tummyActiveSeconds} customTrackers={customTrackers} pumpSession={pumpSession} pumpActiveSeconds={pumpActiveSeconds} pause={pause} resume={resume} pauseTummyTime={pauseTummyTime} resumeTummyTime={resumeTummyTime} pausePumping={pausePumping} resumePumping={resumePumping} />
      {tummySession || pumpSession ? <div className="row hero-actions care-active-actions"><button type="button" className="success end-feed" aria-label={tummySession ? `Stop & save ${careTimerLabel(tummySession, customTrackers)}` : 'Finish & add output'} onClick={() => { if (tummySession) stopTummyTime(); else stopPumping() }}>{tummySession ? `Stop & Save ${careTimerLabel(tummySession, customTrackers)}` : 'Finish & Add Output'}</button><button type="button" className={`active-clear-link ${careClearConfirming ? 'confirming' : ''}`} aria-label={careClearConfirming ? 'Confirm clear active timer' : 'Clear active timer'} onClick={() => careClearConfirming ? clearCare() : setCareClearConfirming(true)}><XCircle size={14} /> {careClearConfirming ? 'Confirm clear' : 'Clear active'}</button></div> : null}
