...
export const HeroPanel = forwardRef<HTMLElement, HeroPanelProps>(function HeroPanel({
  ...
  const [careStopConfirmingFor, setCareStopConfirmingFor] = useState<string | null>(null)
  ...
  return (
    <section className="card hero" ref={ref}>
      ...
      {tummySession || pumpSession ? <div className="row hero-actions care-active-actions"><button type="button" className="success end-feed" aria-label={careStopConfirmingFor === (tummySession?.id ?? pumpSession?.id) ? `Confirm save ${tummySession?.kind === 'sleep' ? 'Sleep' : tummySession ? 'Tummy Time' : 'Pumping'}` : tummySession ? `Stop & save ${tummySession.kind === 'sleep' ? 'Sleep' : 'Tummy Time'}` : 'Finish & add output'} onClick={() => { const active = tummySession ?? pumpSession; if (!active) return; if (careStopConfirmingFor !== active.id) return setCareStopConfirmingFor(active.id); setCareStopConfirmingFor(null); if (tummySession) stopTummyTime(); else stopPumping() }}>{careStopConfirmingFor === (tummySession?.id ?? pumpSession?.id) ? `Confirm save ${tummySession?.kind === 'sleep' ? 'Sleep' : tummySession ? 'Tummy Time' : 'Pumping'}` : tummySession ? `Stop & Save ${tummySession.kind === 'sleep' ? 'Sleep' : 'Tummy Time'}` : 'Finish & Add Output'}</button><button type="button" className={`active-clear-link ${careClearConfirming ? 'confirming' : ''}`} aria-label={careClearConfirming ? 'Confirm clear active timer' : 'Clear active timer'} onClick={() => careClearConfirming ? clearCare() : setCareClearConfirming(true)}><XCircle size={14} /> {careClearConfirming ? 'Confirm clear' : 'Clear active'}</button></div> : null}
