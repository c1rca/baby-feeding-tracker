...
function HeroCue({ session, nextFeedWindowText, nextFeedSideText, hasLastFeed }: HeroCueProps) {
...
function TimerCluster({ session, activeSeconds, activeSide, suggestedSide, pause, resume }: TimerClusterProps) {
  return (
...
function LiveSplit({ session, activeSplit }: LiveSplitProps) {
  if (!session) return null
...
type StartOffsetControlProps = Pick<HeroPanelProps, 'session' | 'startOffsetOpen' | 'startInputMode' | 'startClockText' | 'startMinutesAgo' | 'selectedStartMinutesAgo' | 'setStartOffsetOpen' | 'setStartInputMode' | 'setStartClockText' | 'setStartMinutesAgo'>

function StartOffsetControl({ session, startOffsetOpen, startInputMode, startClockText, startMinutesAgo, selectedStartMinutesAgo, setStartOffsetOpen, setStartInputMode, setStartClockText, setStartMinutesAgo }: StartOffsetControlProps) {
  if (session) return null
...
function DiaperQuickLog({ session, selectedDiapers, availableSelectedDiapers, toggleDiaperSelection, logSelectedDiapers }: DiaperQuickLogProps) {
  return (
...
function AdditionalOptions({ session, additionalOptionsOpen, setAdditionalOptionsOpen, setBottleOpen, setManualOpen, setSession, logMedicine }: AdditionalOptionsProps) {
  return (
...
export const HeroPanel = forwardRef<HTMLElement, HeroPanelProps>(function HeroPanel({
  ...
  return (
    <section className="card hero" ref={ref}>
      ...
      <TimerCluster session={session} activeSeconds={activeSeconds} activeSide={activeSide} suggestedSide={suggestedSide} pause={pause} resume={resume} />
      ...
      <HeroActions session={session} activeSide={activeSide} activeOppositeSide={activeOppositeSide} suggestedSide={suggestedSide} startSession={startSession} switchSide={switchSide} resume={resume} endSession={endSession} clearSession={clearSession} />
      ...
      <AdditionalOptions session={session} additionalOptionsOpen={additionalOptionsOpen} setAdditionalOptionsOpen={setAdditionalOptionsOpen} setBottleOpen={setBottleOpen} setManualOpen={setManualOpen} setSession={setSession} logMedicine={logMedicine} />
    </section>
  )
})
