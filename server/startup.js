...
export const appendStartupStateSnapshot = ({ selectState, selectAllStates = null, appendEventLog, summarizeState, redactError }) => {
  const scopedRows = selectAllStates?.all?.()
  const startupRows = Array.isArray(scopedRows) && scopedRows.length > 0 ? scopedRows : [selectState.get()].filter(Boolean)
  for (const startupRow of startupRows) {
    try {
      ...
      appendEventLog('startup_state_snapshot', { householdId: startupRow.household_id, babyId: startupRow.baby_id, ...summarizeState(entries, session, startupRow.theme || 'light', diapers, medicines, growthMeasurements, babyDob, tummyTimes, tummySession), entries, diapers, medicines, growthMeasurements, babyDob, tummyTimes, tummySession, session })
    } catch (error) {
      appendEventLog('startup_state_snapshot_failed', { householdId: startupRow.household_id, babyId: startupRow.baby_id, error: redactError(error) })
    }
  }
}
